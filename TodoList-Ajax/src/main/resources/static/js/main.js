...
function getTotalCount(){
  ...
  /* GET 방식 fetch() */
  fetch("/todo/totalCount") // 비동기 요청 수행 -> Promise 타입 객체 반환
  .then(response => {    // Promise가 response에 들어감
    ...
    if(response.ok){
      ...
      //                  text(string)(java) 형태로 변환(parsing)
...
function getCompleteCount(){
  ...
  /* 첫 번째 then
    - 비동기 요청 결과(응답)에 따라 어떤 코드를 수행할지 제어
    - 매개변수 response : 
...
function selectTodoList(){
  ...
  .then(result => {
    ...
    // JS 향상된 for문 : for(요소 of 배열){} // 배열을 요소에 저장
...
changeComplete.addEventListener("click", () => {
  ...
  */
  ...
  .then(result => {
    ...
    // 완료 여부 span
    const todoComplete 
        = document.querySelector("#popupComplete");
...
updateBtn.addEventListener("click", () => {
  ...
  // 서버로 제출 되어야 하는 값을 JS 객체인 형태로 묶기
  const obj = {}; // 빈 객체 생성
