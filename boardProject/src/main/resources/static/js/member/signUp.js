...
const initTime = "05:00"; // 인증 초기 시간 지정(5분)
const initMin = 4;  // 초기값 5분에 1초 감소된 분
const initSec = 59; // 초기값 5분에 1초 감소된 초
...
sendAuthKeyBtn.addEventListener("click", () =>{
  ...
  // 1초가 지날때마다 함수 내부 내용이 실행되는 setInterval 작성
  authTimer = setInterval(() => {
    ...
    if(min === 0 && sec === 0){
      ...
      clearInterval(authTimer); // 1초마다 동작하는 setIntervla 멈춤
    ...
    sec--; // 1초가 지날때마다 sec값 1씩감소

  }, 1000); // 1초
});


/* 전달받은 숫자가 10미만(한 자리 수)인 경우
   앞에 0을 붙여서 반환하는 함수 */
...
checkAuthKeyBtn.addEventListener("click", () => {
  ...
  // POST방식으로 보냄, GET 방식은 주소쳐서 들어갈 수 있어서;;
  ...
  // JSON.stringify(객체) : 객체를 JSON으로 변환(문자열화)
  ...
  for(let key in checkObj){ 
    ...
    if( checkObj[key] === false ){ // 유효하지 않은 경우
      ...
      switch(key){
        ...
        case "memberPwConfig" : str = "비밀번호 확인이 일치하지 않습니다"; break;
